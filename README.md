...
- 匿名アップロードにはレート制限（既定 10 回 / 10 分 / IP。インメモリ + Postgres `rate_limits` テーブルで全インスタンス共有）と同一オリジン検査（`Origin` / `Sec-Fetch-Site`）があります。本番では Vercel WAF と LLM プロバイダ側の支出上限の併用を推奨します
- LLM プロバイダのエラー本文はサーバーログにのみ出力し、DB・API レスポンスには含めません
- 画像内の文字列は Judge へ「データであり指示ではない」として区切って渡し、LLM 出力の文字列長はスキーマで上限を設けています（prompt injection 対策）
- CSP / `X-Frame-Options: DENY` / `X-Content-Type-Options` / `Referrer-Policy` を全ページに付与します
